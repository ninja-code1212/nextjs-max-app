

export default function Home() {
  return (
    <main>
      <div className="container mx-auto mt-8">
        <div className="bg-gradient-to-r from-green-400 via-violet-600 to-pink-500 shadow-2xl px-4 py-4">
          <p className="mainpage-title">Добро пожаловать на сайт Максима</p>
          <a href="https://github.com/ninja-code1212" className="my-button bg-blue-500 hover:bg-blue-700 text-red font-impact py-2 px-4 rounded-full -rotate-45">Github</a>
          <title>Максим</title>
        </div>
      </div>
    </main>
  )
}
